...
output:
```bash
ubuntu@selfhostlab:~$ git clone https://github.com/plausible/community-edition .
...
2. Replace the `BASE_URL` with the domain which you want to use for serving the analytics. In my case, I will be using `analytics.selfhostlab.dev`.

```bash
BASE_URL=https://analytics.selfhostlab.dev
```
...
output:
```bash
ubuntu@selfhostlab:~/analytics$ openssl rand -base64 48
...
plausible:
    labels:
      virtual.host: "analytics.selfhostlab.dev"
      virtual.port: "8000"
      virtual.tls-email: "lab@selfhostlab.dev"
```
...
1. Let's configure the DNS records for the domain which we are going to use to serve the Analytics. For this, we will create an A record pointing your domain to the IP address of the server. In my case, I will be using `analytics.selfhostlab.dev` as the domain to point to 159.65.157.121.
